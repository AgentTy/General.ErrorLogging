...
(function (ns) {
    ...
    var FiltersCookie = 'GenErrLogFilters';
    var AccessCode = '';
        ...
        if (ErrorLogger.Filters && ErrorLogger.Filters.length > 0) {
            ...
            $.each(ErrorLogger.Filters, function (index, filter) {
                ...
                if (filter.EventFilter.minseverity > 0) {
                    if (!objEventContext.Severity)
                        eventMatch = false;
                    else if (objEventContext.Severity < filter.EventFilter.minseverity)
                        eventMatch = false;
                }
    ...
    ns.StoreEvent = function(objEventContext, objFilterContext)
    {
        ...
        //POST to API
        try {
            var objErrHistory = null;
            try {
                if (ns.LocalHistoryEnabled)
                    objErrHistory = simpleHistory(ns.EventHistory);
            } catch (e) { if (typeof console !== 'undefined' && ns.DoConsole) console.log(e); }
    ...
    ns.LoadFilters = function () {
        if (UseLocalStorage) {
            var filters = loadFiltersFromLocalStorage();
            if (!filters || filtersAreExpired(filters)) {
    ...
    function loadFiltersFromLocalStorage() {
        ...
        filterContext = localStorage.getItem(FiltersCookie);
        try {
            filterContext = (filterContext ? JSON.parse(filterContext) : null)
        } catch (e) {
            filterContext = null;
            if (typeof console !== 'undefined' && ns.DoConsole) console.log(e);
        }
        if (filterContext && typeof filterContext.Filters !== 'undefined') {
            ErrorLogger.Filters = filterContext.Filters;
        }
        else if (filterContext) {
            if (typeof console !== 'undefined' && ns.DoConsole) console.log('filterContext load failed', filterContext);
            return null;
        }
        return filterContext;
    }

    function loadFiltersFromServer() {
    ...
    function filtersAreExpired(filterContext) {
        try {
            var age = new Date() - new Date(filterContext.Time);
            return age > ns.PollingInterval;
        }
        catch (e) {
            if (typeof console !== 'undefined' && ns.DoConsole) console.log(e);
            return true;
    ...
    function saveFilters(filters) {
        var filterContext = { Filters: filters, Time: new Date() }
        localStorage.setItem(FiltersCookie, JSON.stringify(filterContext));
    }
