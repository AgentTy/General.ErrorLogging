...
namespace General.ErrorLogging.Data
{
    ...
    public class ErrorOther
    {
        ...
        public static long StoreEvent(Model.ErrorOther objModel, FilterContext filterContext, EventHistoryContext historyContext)
		{
            ...
            if (!String.IsNullOrWhiteSpace(objModel.ClientID)) cmd.Parameters.AddWithValue("@ClientID", objModel.ClientID);
            ...
            if (!String.IsNullOrWhiteSpace(objModel.UserType)) cmd.Parameters.AddWithValue("@UserType", objModel.UserType);
            if (!String.IsNullOrWhiteSpace(objModel.UserID)) cmd.Parameters.AddWithValue("@UserID", objModel.UserID);
        ...
        public static long StoreEvent(General.ErrorLogging.Client.RecordEventDataContext request)
        {
            ...
            if (!String.IsNullOrWhiteSpace(request.AppContext.ClientID)) cmd.Parameters.AddWithValue("@ClientID", request.AppContext.ClientID);
            ...
            if (!String.IsNullOrWhiteSpace(request.AppContext.UserType)) cmd.Parameters.AddWithValue("@UserType", request.AppContext.UserType);
            if (!String.IsNullOrWhiteSpace(request.AppContext.UserID)) cmd.Parameters.AddWithValue("@UserID", request.AppContext.UserID);
        ...
        public static long StoreEvent(int AppID, Environment.EnvironmentContext EnvironmentContext, string ClientID, ErrorOtherTypes EventType, int? intSeverity, string ExceptionType, string CodeMethod, string CodeFileName, int CodeLineNumber, int CodeColumnNumber, string ErrorCode, string EventName, string EventDetail, string EventURL, string UserAgent, string UserType, string UserID, int? CustomID, string AppName, string MachineName, string Custom1, string Custom2, string Custom3, int? Duration, FilterContext filterContext, EventHistoryContext historyContext)
        {
            ...
            if (!String.IsNullOrWhiteSpace(ClientID)) cmd.Parameters.AddWithValue("@ClientID", ClientID);
            ...
            if (!String.IsNullOrWhiteSpace(UserType)) cmd.Parameters.AddWithValue("@UserType", UserType);
            if (!String.IsNullOrWhiteSpace(UserID)) cmd.Parameters.AddWithValue("@UserID", UserID);
