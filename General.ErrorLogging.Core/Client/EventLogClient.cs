    ...
    public class EventLogClient
    {
                ...
                else
                {
                           ...
                           (async () =>
                           {
                               ...
                               try
                               {
                                   ...
                                   if (httpResult.IsSuccessStatusCode)
                                   {
                                       ...
                                       response = await JsonSerializer.DeserializeAsync<EventReporterResponse>(await httpResult.Content.ReadAsStreamAsync(), options).ConfigureAwait(false);
                                   }
                                   else
                                   {
                    ...
                    else
                    {
                        ...
                        if (httpResult.IsSuccessStatusCode)
                        {
                            ...
                            JsonSerializerOptions options = new JsonSerializerOptions();
                            options.IncludeFields = true;
                            response = await JsonSerializer.DeserializeAsync<EventReporterResponse>(await httpResult.Content.ReadAsStreamAsync(), options);
                        }
                        else
                        {
                            response.Success = false;
        ...
        protected static async Task<HttpResponseMessage> StoreEventViaAPI(Uri uriEndpoint, string apiPath, RecordEventDataContext request)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                JsonSerializerOptions options = new JsonSerializerOptions();
                options.IncludeFields = true;
                var content = JsonContent.Create<RecordEventDataContext>(request, new MediaTypeWithQualityHeaderValue("application/json"), options);
                return await client.PostAsync(apiPath, content).ConfigureAwait(false);

                //Won't work until I change class fields to properties!
                //return await client.PostAsJsonAsync<RecordEventDataContext>(apiPath, request).ConfigureAwait(false);
            }
        }
        ...
        protected static HttpResponseMessage StoreEventViaAPI_Synchronous(Uri uriEndpoint, string apiPath, RecordEventDataContext request)
        {
            using (var client = new HttpClient())
            {
                ...
                options.IncludeFields = true;
                var content = JsonContent.Create<RecordEventDataContext>(request, new MediaTypeWithQualityHeaderValue("application/json"), options);
                return client.PostAsync(apiPath, content).Result;

                //return client.PostAsJsonAsync<RecordEventDataContext>(apiPath, request).Result;
            }
        }
...
";
        ...
        public static async Task<bool> SendEmail(General.Model.EmailAddress FromEmail, List<General.Model.EmailAddress> Recipients, string Subject, string Body, bool IsHtml = false, List<string> Attachments = null, bool SendWithBCC = true)
        {
            ...
            else if (Recipients.Count == 1)
            {
                ...
                if (String.IsNullOrWhiteSpace(objEmail.Name))
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        var content = JsonContent.Create<General.ErrorLogging.Data.LoggingFilter.GetActiveFiltersDataContext>(request, new MediaTypeWithQualityHeaderValue("application/json"), options);
                        HttpResponseMessage httpResult = await client.PostAsync(apiPath, content).ConfigureAwait(false);
                        //HttpResponseMessage httpResult = await client.PostAsJsonAsync<General.ErrorLogging.Data.LoggingFilter.GetActiveFiltersDataContext>(apiPath, request).ConfigureAwait(false);
                        
                        if (httpResult.IsSuccessStatusCode)
                        {
                            //var filters = await httpResult.Content.ReadAsAsync<List<LoggingFilterRemoteServerView>>().ConfigureAwait(false);
                            var filters = await JsonSerializer.DeserializeAsync<List<LoggingFilterRemoteServerView>>(await httpResult.Content.ReadAsStreamAsync(), options).ConfigureAwait(false);
